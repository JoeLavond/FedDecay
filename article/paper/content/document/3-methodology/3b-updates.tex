...
Equation~\ref{eq: theta} defines the standard local update equation for the FedAvg ($K > 1$) or FedSGD ($K = 1$) procedure, which are produced by users performing stochastic gradient descent for $K$ updates.
\begin{align}
...
The new shared model becomes the averaged element-wise parameters from the locally trained user models.
\begin{align}
...
This expression is the change that a user's local training made to the shared model.
...
\edit{We refer to this component of a user's change as the \textbf{method gradient}, $g_{Method}$, due to its formulation and use in Equation~\ref{eq: recursive}.}
